    ...
    if (isDryRun) {
      ...
      return NextResponse.json({
        ...
        preview: validationResult.data ? {
          totalRecords: Object.values(validationResult.data.metadata.recordCounts).reduce((a, b) => a + b, 0),
          recordCounts: validationResult.data.metadata.recordCounts,
          wouldImport: Object.values(validationResult.data.metadata.recordCounts).reduce((a, b) => a + b, 0)
        } : null,
        warnings: validationResult.warnings
      });
    }
