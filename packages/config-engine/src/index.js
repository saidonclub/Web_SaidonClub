...
const database_1 = require("@saidonclub/database");
...
class ConfigManager {
     ...
     */
    async set(key, value, userId, reason) {
        const config = await database_1.prisma.systemConfig.findUnique({ where: { key } });
        if (!config)
            throw new Error(`Configuración '${key}' no existe en el schema.`);
        ...
        await database_1.prisma.configHistory.create({
            data: {
                configId: config.id,
                key,
    ...
    async rollback(key, historyId, userId) {
        const history = await database_1.prisma.configHistory.findUnique({ where: { id: historyId } });
    ...
    async validateDependencies(key, value) {
        const config = await database_1.prisma.systemConfig.findUnique({ where: { key } });
        if (!config)
            return;
        // Si estamos DESACTIVANDO, verificar que nadie dependa de esta key
        if (value === false || value === 'false') {
            const dependents = await database_1.prisma.systemConfig.findMany({
                where: {
        ...
        if (value === true || value === 'true') {
            for (const depKey of config.dependencies) {
                const dep = await database_1.prisma.systemConfig.findUnique({ where: { key: depKey } });
